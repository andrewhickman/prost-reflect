...
impl DescriptorPool {
    /// Create a [`DescriptorPool`] from a [`FileDescriptorSet`].
    ///
    ...
    pub fn new(file_descriptor_set: FileDescriptorSet) -> Result<Self, DescriptorError> {
        let inner = DescriptorPool::from_raw(file_descriptor_set)?;
        Ok(DescriptorPool {
            inner: Arc::new(inner),
        })
    }
    ...
    /// Gets a reference to the [`FileDescriptorSet`] wrapped by this [`DescriptorPool`].
